/* Click-step scene engine for the Casino (ε-greedy) viz.
   ...
   inside the scene, e.g. step the action history forward), false to let the driver
   advance the scene. */
(function () {
  // TRIMMED FOR THE FDD COURSE, 2026-08-22 (Carlos: "one scene only, just the
  // basics"). The SML original has five: the free play board, explore or exploit,
  // epsilon greedy, the trade off, and a recap. Only the board survives here, which
  // is the bandit problem itself and nothing else. The scene engine takes a list of
  // one; the arrows then have nowhere to go, which is why they are hidden below.
  const SCENES = [
  ...
  window.CasinoViz = { goTo, getCurrentScene: () => current, sceneTitles: SCENE_TITLES };
})();
